...
use IntegrationHelper\IntegrationMasterLaravel\Datagrids\DataGridExport;
use IntegrationHelper\IntegrationMasterLaravel\Datagrids\Action;
use IntegrationHelper\IntegrationMasterLaravel\Datagrids\Column;
use IntegrationHelper\IntegrationMasterLaravel\Datagrids\Enums\ColumnTypeEnum;
use IntegrationHelper\IntegrationMasterLaravel\Datagrids\MassAction;
...
abstract class DataGrid
{
    ...
    /**
     * Use only if you have id search criterias in yours $queryBuilder
     *
     * @var bool
     */
    protected $enableFastMode = false;
    ...
    public function addMassAction(array $massAction): void
    {
        $this->massActions[] = new MassAction(
            ...
            title: $massAction['title'] ?? $massAction['label'],
            method: $massAction['method'],
            url: $massAction['url'] ?? $massAction['action'],
     ...
     */
    public function processRequestedFilters(array $requestedFilters, \Illuminate\Database\Query\Builder $queryBuilder = null)
    {
     ...
     * @return \Illuminate\Database\Query\Builder
     */
    public function processRequestedSorting($requestedSort, \Illuminate\Database\Query\Builder $queryBuilder = null)
    {
        if (!$this->sortColumn) {
        ...
        return $queryBuilder ?
            $queryBuilder->orderBy($requestedSort['column'] ?? $this->sortColumn, $requestedSort['order'] ?? $this->sortOrder) :
     ...
     */
    public function processRequestedPagination(
        $requestedPagination,
        \Illuminate\Database\Query\Builder $queryBuilder = null
    ): LengthAwarePaginator
    {
        ...
        return $queryBuilder->paginate(
            ...
            ['*'],
            'page',
            $this->enableFastMode ? 1: ($requestedPagination['page'] ?? 1)
        );
    }
     ...
     * @return array
     */
    protected function getTotal(array $paginator): array
    {
        return [
            'from'             => $paginator['from'],
            'to'               => $paginator['to'],
    ...
    public function formatData(): array
    {
        ...
        $pagesData = $this->getTotal($paginator);
        ...
        return [
            ...
            'meta'         => [
                ...
                'per_page_options' => [10, 20, 30, 40, 50, 100, 200, 500],
                'per_page'         => $pagesData['per_page'],
